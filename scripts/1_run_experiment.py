...
def run_experiment(
    ...
    if 'lora' in arms:
        ...
        results = run_lora_arm(
            ...
            num_labels=config.num_labels,
            batch_size=batch_size,
            learning_rate=lora_config.get('learning_rate', lora_config.get('lr', 3e-4)),
            weight_decay=lora_config['weight_decay'],
            max_epochs=lora_config['max_epochs'],
            early_stop_patience=lora_config.get('early_stop_patience', lora_config.get('patience', 2)),
            early_stop_min_delta=lora_config.get('early_stop_min_delta', lora_config.get('min_delta', 0.001)),
