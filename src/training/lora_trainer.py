"""
Arm C: LoRA Fine-tuning

1. Loads baseline_start checkpoint
2. Applies LoRA adapters (rank=8, alpha=16)
...
def create_lora_model(
    ...
    Args:
        ...
        target_modules: Modules to apply LoRA (default: q/k/v/o projection)
    ...
    print_once(f"  Rank: {lora_rank}")
    print_once(f"  Alpha: {lora_alpha}")
    print_once(f"  Dropout: {lora_dropout}")
    print_once(f"  Target modules: {target_modules}")
...
def run_lora_arm(
    ...
    """
    Arm C: LoRA fine-tuning with early stopping
    ...
    """
    print_once("\n" + "=" * 60)
    print_once("ARM C: LORA FINE-TUNING")
    print_once("=" * 60)
    ...
    results = {
        ...
        'notes': f'LoRA (rank={lora_rank}, alpha={lora_alpha})'
    }

    # Print results
    print_once("\n" + "=" * 60)
    print_once("ARM C RESULTS")
    print_once("=" * 60)
